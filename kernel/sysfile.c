...
sys_mprotect(void)
{
  ...
  printf("sys_mprotect: addr=%p, len=%d, prot=%d\n", (void*)addr, len, prot);
  ...
  if (found == -1) {
    printf("mprotect: address not in any VMA\n");
    return -1;
  }
  ...
  // Cập nhật quyền trong VMA
  vma->prot = prot;
  
  // Duyệt từng trang và cập nhật PTE
  for (uint64 va = start; va < end; va += PGSIZE) {
    pte_t *pte = walk(p->pagetable, va, 0);
    
    if (pte && (*pte & PTE_V)) {
      // Giữ nguyên cờ cơ bản, chỉ thay đổi quyền
      int perm = (*pte) & ~(PTE_R | PTE_W | PTE_X);
      perm |= PTE_U;  // luôn có user flag
      ...
      *pte = (*pte & ~(PTE_R | PTE_W | PTE_X)) | perm;
      ...
      printf("mprotect: updated va=%p, perm=%x\n", (void*)va, perm);
    }
  }
